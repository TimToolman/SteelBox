...
const res = await open(browser, { path: 'admin', email: 'mvp.admin@ntlsb.com', width: 1440, height: 950 })
...
const hq = await open(browser, { path: 'admin', email: 'hq@ntlsb.com', width: 1440, height: 950 })
...
ok(/shipper@ntlsb.com/.test(cView), 'listing the existing account')
ok(/CAN SIGN IN/.test(cView), 'with its access state')
await hq.locator('input[placeholder="Full name"]').fill('Ana Osei')
await hq.locator('input[placeholder="email@line.com"]').fill('ana.osei@ntlsb.com')
await hq.getByRole('button', { name: 'Invite' }).click()
await hq.waitForTimeout(1000)
ok(/ana.osei@ntlsb.com/.test(await text(hq)), 'inviting adds the contact')
...
await hq.locator('input[placeholder="user@ntlsb.com"]').fill('larry@nowhere.dev')
