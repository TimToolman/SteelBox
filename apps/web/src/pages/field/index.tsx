// ============================================================
// MVP Container Field App — Mobile web app for drivers + inspectors
...
export default function FieldAppPage() {
  useFavicon('favicon-field.svg', 'MVP Container Field App')
  ...
  const sendReceipt = (job: Job) => {
    ...
    if (w) {
      ...
      w.document.write(`<html><head><title>MVP Container Receipt ${job.sku}</title><style>body{font:14px -apple-system,sans-serif;padding:28px;color:#0D0E12}h1{font-size:20px;margin:0 0 2px}.sub{color:#6B7280;font-size:12px}.row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee}.k{color:#6B7280}b{font-family:monospace}</style></head><body><h1><span style="color:#2B7FD4">MVP</span> <span style="color:#E65100">Container</span></h1><div class="sub">${KIND_META[job.kind].label} receipt · ${now}</div><div style="margin-top:20px"><div class="row"><span class="k">Container</span><b>${job.sku}</b></div><div class="row"><span class="k">Customer</span><span>${job.customer || '—'}</span></div><div class="row"><span class="k">Location</span><span>${job.address || '—'}</span></div><div class="row"><span class="k">Field rep</span><span>${me?.name ?? ACTOR}</span></div><div class="row"><span class="k">Status</span><span>Completed</span></div></div><p class="sub" style="margin-top:24px">A copy has been emailed to ${job.email || 'the customer'} and the MVP Container admin portal.</p><button onclick="window.print()" style="margin-top:14px;padding:10px 18px;border:none;border-radius:999px;background:#0057B8;color:#fff;font-weight:700;cursor:pointer">Save as PDF</button></body></html>`)
      w.document.close()
    }
  ...
  const topBar = (
    <div style={{ background: 'linear-gradient(135deg,#0057B8,#003882)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
      ...
      <span style={{ fontSize: '14px', fontWeight: 700 }}><span style={{ color: '#90C4FF' }}>MVP&nbsp;</span><span style={{ color: '#E65100' }}>Container</span></span>
      <button
