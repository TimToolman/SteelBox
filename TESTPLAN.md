...
| G-2c | Profile close returns home | On the home page, click the profile icon, then Close on the sheet | The sheet opens over /shop (deep link); Close returns to the page that launched it — the main website — instead of stranding the visitor on /shop. Signing in instead stays in the marketplace; a profile opened from within /shop closes in place | Manual only |
